...
```{r}
weather <-list(
          list(day = "Thursday", "temp_celc" = 18, "rain_%" = 50, "wind" = 10),
          list(day = "Friday", "temp_c" = 18, "rain_%" = 10, "wind" = 18),
          list(day = "Sat", "temp_c" = 17, "rain_%" = 10, "wind" = 15)
          )
 weather
...
#Answer 2
```{r}
forecast_aberdeen <- list(
                Thursday = list('temp' = 18, 'rain' = 5, 'wind' = 10),
                Friday = list('temp' = 15, 'rain' = 10, 'wind' = 8),
                Saturday = list('temp' = 17, 'rain' = 10, 'wind' = 15)) 
forecast_aberdeen
```
**Question 2**
Using the list you've just made find:
  i. The chance of rain tomorrow -- $Thursday$rain [1] 5%

  ii. The wind speed on the day after tomorrow -- $Friday$wind [1] 8mph
  
  iii. The temperature tomorrow -- $Friday$temp [1] 15C
```{r}
forecast_Aberdeen-list$Thursday$chance_of_rain
forecast_3_Aberdeen_list$Friday$wind_speed
forecast_Aberdeen_list$Thursday$temperature
```
...
```{r}
fc_fortnight_v <- c(18L, 15L, 18L, 22L, 20L, 18L, 18L, 19L, 22L, 22L, 21L, 20L, 21L)

fc_fortnight <- matrix(fc_fortnight_v,
                       nrow = 2, 
                       ncol = 7, 
                       byrow = TRUE)
fc_fortnight            
